...
for (industry in unique(df$industry)) {
  # subset the df to only include rows for the current industry
  df_industry <- df[df$industry == industry,]
  avg <- calculate_average(df_industry$total_laid_off)
  # replacing "NA" with average
  df_industry$total_laid_off[is.na(df_industry$total_laid_off)] <- avg
  # update the original df with new
  df[df$industry == industry,] <- df_industry
}
df
```
