---
title: "SF Library Usuage Analysis"
...
library_data%>%
  ...
  ggplot( aes(x=year_registered, group=age, fill=age)) +
    ...
    theme_ipsum()
```
As expected people with ages 60 and higher have been registered since 2003. We can see that the young children from the age of 0-9 years have started registering since 2012, which shows they are curious readers or their parents would've enrolled them to read books and gain information.  
...
ggplot(library_data, aes(x=branch, y=year_registered, fill=branch)) + 
   ...
   labs(x='Year Registered',y='Branch')
```
...
This shows year of registration of different types of patrons residing in and out of San Franciso.
```{r}
...
This shows year of registration for different age groups residing in and out of San Franciso.
```{r}
# Using Small multiple
...
I tried to learn, have fun with it, and incorporate all types of plots, specially histograms, density plots and facet_wrap which made it easier to see observations. If we had more information about the data from other parts of California or USA, I could have found trends for a much higher population using these visualizations. Also, we need more information about the branches, as only geographical information was available on the website. Apart from that, some information about the different categories of books people are renewing, which age groups like which categories more, that would be an interesting research to do.
...
We also figured, that according to the area code/pincode, "94102" is the most popular area for checkouts and renewals. On looking in more detail about that area, we figured most checkouts were made by ADULTS of age 45-54 years and 54-59 years and SENIORS of 64-74 years. There are also a few checkouts by STAFF members with ages 0-9 years, which is weird. There are also some patrons who reside outside the SF county, but they are lesser in numbers than the ones in SF. The registry with SF Public library over the years for people residing in and out of the county can be seen the graphs above, they have been grouped by Age Range and Patron Types. 
