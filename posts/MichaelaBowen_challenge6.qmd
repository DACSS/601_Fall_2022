...
total_sales_weekday <-transactions_sept22_oct20_working %>%
  ...
  mutate(total_sales = sum(price),
         avg_sales = case_when(
           weekdayLabel == "Sun" ~ total_sales/((n_distinct(week(date)))-1),
           weekdayLabel == "Mon" ~ total_sales/((n_distinct(week(date)))-1),
           weekdayLabel == "Tue" ~ total_sales/((n_distinct(week(date)))-1),
           weekdayLabel == "Wed" ~ total_sales/((n_distinct(week(date)))-1),
           weekdayLabel == "Thu" ~ total_sales/(n_distinct(week(date))),
           weekdayLabel == "Fri" ~ total_sales/((n_distinct(week(date)))-1),
           weekdayLabel == "Sat" ~ total_sales/((n_distinct(week(date)))-1)
  ))%>%
...
ggplot( mapping = aes(x = weekdayLabel, y = avg_sales, color = FirstName), size = 3,) +
  ...
  scale_y_continuous(labels = dollar, limits = c(0,8000)) +
  ylab("Average Sales ($)") +
  xlab("Weekday") +
