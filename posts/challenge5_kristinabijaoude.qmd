...
```{r}
# create data
x <- c(AB_NYC_2019$`Minimum Number of Nights`)
y <- c(AB_NYC_2019$`Price per Night ($)`)
...
```{r}
# create data
NYC_Borough <- c(AB_NYC_2019$Borough)
Price <- c(AB_NYC_2019$`Price per Night ($)`)

# create violin plot
 ggplot(AB_NYC_2019, aes(x=NYC_Borough, y=Price, fill=Price)) +
   scale_y_continuous(label = scales::dollar) +
  geom_violin()
```
