...
I realize now that since my data includes only the data where there are more disagreements than agreements of identification, I cannot compare this group to the larger control group. Consideration of my denominator is key when selecting my own data.

STUCK HERE.
```{r}
#calculate proportions of families by year
#obs_clean %>%
 # mutate(obs_clean, year = year("time_observed_at")) %>%
#group_by(year)
# find top 10 families
#obs_clean %>%
#  select(obs_clean$time_observed_at, taxon_family_name) %>%
#  group_by(year) %>%
#  arrange(desc(taxon_family_name))
