...
I think I may need to change the data in `time_zone` to something that lubridate will recognize, and then concatenate with each date variables and then run `ymd()` on those date fields. Alternatively, I could truncate `time_zone` to exclude "+0000" and then use the `time_zone` data independently, and then connect it with the resulting ymd dates.
