...
```{r}
G9Science_Cat_StateDiff<-G9Science_StudentItem%>%
...
G9Science_Cat_StateDiff
   ggplot(G9Science_Cat_StateDiff, aes(fill = `sReporting Category`, y=`avg_RT_State_Diff`, x=`sReporting Category`)) +
...
```{r}
G9Science_Cat_Box <-G9Science_StudentItem%>%
  select(sitem, sitem_score, `sReporting Category`, `sItem Possible Points`, `State Percent Points`, `RT-State Diff`, IEP)%>%
...
```
```{r}
G9Science_Density <-G9Science_StudentItem%>%
  select(sitem, sitem_score, `sReporting Category`, `sItem Possible Points`, `State Percent Points`, `RT-State Diff`, IEP)%>%
...
`
```{r}
G9Science_Histo <-G9Science_StudentItem%>%
  select(sitem, sitem_score, `sReporting Category`, `sItem Possible Points`, `State Percent Points`, `RT-State Diff`, IEP)%>%
  ggplot(aes(x = `RT-State Diff`, color = `sReporting Category`, fill = `sReporting Category`)) + 
