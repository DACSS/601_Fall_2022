...
```
Where were the least points earned relative to other students in the state?
...
to their peers in the state on questions from the `sReporting Category` Motion and Forces.
...
```
Since Motion and Forces questions account for 50% of the points available on the test, our students "lost" in total the most points relative to their peers in the state in this category.
...
Here we see the distribution of `RT-State Diff` by reporting category and question item. We can see generally that Motion and Forces seems to have the highest variablility. It would be worth looking at the specific question strands with the Physics Teachers. (It would be helpful to add item labels to the dots)
```{r}
...
`
Where did Rising Tide Students lose the most points? We can see that in total students 
lost the most points on items from Motion and Forces
```{r}
G9Science_Cat_Loss<-G9Science_StudentItem%>%
  ...
  summarise(sum_RT_points_lost = sum(`sitem Possible Points`-`sitem_score`, na.rm=TRUE),
            RT_points_lost_per_student = sum(`sitem Possible Points`-`sitem_score`, na.rm=TRUE)/69)
...
from questions with calculate and half do not. In the Motion and Forces category, more points 
...
G9Science_Calc_PointsAvail
 ...
 G9Science_Calc_PointsAvail_Stacked <- G9Science_Calc%>%
   ggplot(aes(fill=key_word, y = `sitem Possible Points`, x=`sReporting Category`)) +
...
```
Here we can see the diparity in the `RT-State Diff` by Item when we consider the term "Calculate"
...
G9Science_Calc_Box
```
As we can see, that a student's performance on their 8th Grade Math MCAS is strongly associcated with their performance on the Introductory Physics exam. This suggests, we should use prior MCAS and current STAR Math testing data to select students for intervention.
```{r}
...
their points on questions involving "Calculate". In the general, classroom, to raise the level 
of rigor, students should spend more time on calculation based activities.
```{r}
# G9 Points Lost
G9Sci_StudentCalcPerflev<-G9Science_StudentItem%>%
  select(gender, sitem, sitem_score, `sitem Desc`, `sitem Possible Points`, `State Percent Points`, IEP, `RT-State Diff`, `sReporting Category`, `sperflev`)%>%
  mutate( key_word = case_when(
     !str_detect(`sitem Desc`, "calculate|Calculate") ~ "Non-Calc",
     str_detect(`sitem Desc`, "calculate|Calculate") ~ "Calc"))%>%
  group_by(`sperflev`, `key_word`)%>%
  summarise(total_points_lost = sum(`sitem_score`-`sitem Possible Points`, na.rm = TRUE))
...
G9Sci_StudentCalc<-G9Science_StudentItem%>%
  ...
  mutate( key_word = case_when(
     !str_detect(`sitem Desc`, "calculate|Calculate") ~ "Non-Calc",
     str_detect(`sitem Desc`, "calculate|Calculate") ~ "Calc"))%>%
  group_by(`sReporting Category`, `key_word`, `sperflev`)%>%
  summarise(total_points_lost = sum(`sitem_score`-`sitem Possible Points`, na.rm = TRUE))%>%
 ggplot(aes(fill=`key_word`, y=total_points_lost, x=`sReporting Category`)) + geom_bar(position="dodge", stat="identity") + coord_flip()+
   labs(subtitle ="Calculate" ,
       y = "Sum Points Lost",
       x= "Reporting Category",
...
G9Sci_StudentCalcPerflev%>%
 ggplot(aes(fill=`key_word`, y=total_points_lost, x=`sperflev`)) + geom_bar(position="fill", stat="identity") +
   labs(subtitle ="Calculate" ,
       ...
       x= "Performance Level",
       title = "Points Lost by Key Word and Performance Level",
      caption = "2022 HS Introductory Physics MCAS")
...
G9Sci_StudentCalcDis<-G9Science_StudentItem%>%
  ...
  mutate( key_word = case_when(
     !str_detect(`sitem Desc`, "calculate|Calculate") ~ "Non-Calc",
     str_detect(`sitem Desc`, "calculate|Calculate") ~ "Calc"))%>%
  group_by(`sReporting Category`, `key_word`, `IEP`)%>%
  summarise(total_points_lost = sum(`sitem_score`-`sitem Possible Points`, na.rm = TRUE))%>%
 ggplot(aes(fill=`key_word`, y=total_points_lost, x=`sReporting Category`)) + geom_bar(position="dodge", stat="identity")+
...
G9Sci_StudentCalcDis<-G9Science_StudentItem%>%
  ...
  mutate( key_word = case_when(
     ...
     str_detect(`sitem Desc`, "calculate|Calculate") ~ "Calc"))%>%
  group_by(`sReporting Category`, `key_word`, `IEP`)%>%
  summarise(sum_points_lost = sum(`sitem_score`-`sitem Possible Points`, na.rm = TRUE))%>%
 ggplot(aes(fill=`key_word`, y=sum_points_lost, x=`sReporting Category`)) + geom_bar(position="fill", stat="identity")+
  facet_wrap(vars(IEP))+ coord_flip()+
   labs(subtitle ="Calculate" ,
       y = "Percent Points Lost",
       x= "Reporting Category",
       title = "Percent Points Lost by Key Word",
      caption = "2022 HS Introductory Physics MCAS")
