...
bar_list <- bnb_plot %>% ggplot(aes(x=`Borough`, fill=`Room Type`))+
                                geom_bar(position='dodge')+
                                  scale_fill_brewer(palette = 'PRGn')+
                                    scale_y_continuous(breaks=seq(0,8000, by=2000))+
                                   theme_dark()+
                                        labs(title= "2019 NYC Air Bnb Listing Count by Borough", x= "Borough", y= "Count", caption= "visualized by Room Type")
                           
bar_list

```
Using a bar plot with `position = dodge` allows us to view Lisiting counts for each Borough by Room Type, with every Room Type having an individual bar.
:::

:::{.callout-note collapse="true"}

## `Price` by `Borough` Bar
```{r}
...
bar_price <- bnb_price %>% ggplot(aes(x=`Price Rating`, fill=`Borough`))+
                            geom_bar(position ='fill')+
                                  scale_fill_brewer(palette = 'PRGn')+
                                   theme_dark()+
                                      coord_flip()+
                                        labs(title= "2019 NYC Air Bnb Listings by Price Rating", x= " Nightly Price", y= "Frequency", caption= "visualized by Borough")
                ...
                geom_bar(position = 'stack')+
                    scale_fill_brewer(palette = 'PRGn')+
                                    scale_y_continuous(breaks=seq(0,7500, by=1500))+
                                        theme_dark()+
                                          coord_flip()+
                                        labs(title= "2019 NYC Air Bnb Listing Count by Availabilty", x= " Availability", y= "Count", caption= "visualized by Price Rating")
...
bnb_plot %>% 
                ggplot(aes(x=`Review Count`, y=Price))+
                geom_point(aes(color=Borough))+
                  coord_cartesian(ylim=c(0,1000))+
                    scale_fill_brewer(palette = 'PRGn')+
                                    scale_y_continuous(breaks=seq(0,1500, by=150))+
                                   theme_minimal()+
                                        labs(title= "2019 NYC Air Bnb Price by Review Count ", x= "Review Count", y= "Nightly Price", caption= "visualized by Borough")
